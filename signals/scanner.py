...
def composite_verdict(score: int, signals: list[SignalResult] | None = None) -> str:
    ...
    signals. Sell/Strong Sell is narrower: it's driven only by RSI and the
    Double Top/Bottom pattern, regardless of what the other four signals
    say — both bearish gives Strong Sell, either one alone gives Sell.
    Pass `signals` to enable that gate; without it, this falls back to
    score-only thresholds for both directions."""
